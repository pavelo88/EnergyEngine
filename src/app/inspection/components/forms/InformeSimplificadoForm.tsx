  ...
  const handleSave = async () => {
    ...
    const saveDataToLocal = async (synced: boolean, firebaseId?: string) => {
        ...
        if (!synced) {
            ...
            alert(`Informe Simplificado guardado con éxito. ID: ${firebaseId}`);
        }
    };
        ...
        <main className="p-4 md:p-6 space-y-8 pb-40">
            ...
            <section className="bg-white p-6 md:p-10 rounded-lg shadow-sm space-y-4 border border-slate-100">
                ...
                <div className="grid grid-cols-1 gap-y-4">
                    ...
                    <div key={it} className={`p-4 rounded-lg flex justify-between items-center transition-all border ${formData.recambios_checklist[it] ? 'bg-primary/10 border-primary/20' : 'bg-slate-50/50 border-slate-100'}`}>
